    ...
    <div class="content-search">
      <div class="page-section-demo">
        <div class="search-outer">
          <input class="search-input" v-model="pageConfig.place" placeholder="搜索" />
  ...
  export default {
      ...
      pageConfig: {
        handler: async function tmp () {
          ...
          try {
            ...
            let pageQuery = {
              place: this.pageConfig.place,
              pageNo: this.pageStatus.currentPageIndex,
              pageSize: 10,
              level: (levelEnum.indexOf(this.pageConfig.navList[0]) === -1) ? undefined : levelEnum.indexOf(this.pageConfig.navList[0]),
              workOrderStates: (statusEnum.indexOf(this.pageConfig.navList[1]) === -1) ? undefined : statusEnum.indexOf(this.pageConfig.navList[1])
            }
            if (role === 'wyry') {
              pageQuery.userId = userInfo.id
              pageQuery.maintenanceId = userInfo.deptId
              pageQuery.institutionType = 2
            } else if (role === 'wyjl') {
              pageQuery.maintenanceId = userInfo.deptId
              pageQuery.institutionType = 2
            } else if (role === 'wbry') {
              pageQuery.userId = userInfo.id
              pageQuery.maintenanceId = userInfo.deptId
              pageQuery.institutionType = 1
            } else if (role === 'wbjl') {
              pageQuery.maintenanceId = userInfo.deptId
              pageQuery.institutionType = 1
            }
            this.pageData.problemList = (await this.$request('/AppWarning/AppWarning', 'GET', pageQuery)).data.data.rows.map(item => ({
              type: item.type,
    ...
    methods: {
      ...
      async handleReachBottom () {
        ...
        if (role === 'wyry') {
          pageQuery.userId = userInfo.id
          pageQuery.institutionType = 0
        } else if (role === 'wyjl') {
          pageQuery.propertyId = userInfo.deptId
          pageQuery.institutionType = 2
        } else if (role === 'wbry') {
          pageQuery.userId = userInfo.id
          pageQuery.institutionType = 1
          ...
          pageQuery.maintenanceId = userInfo.deptId
          pageQuery.institutionType = 3
        }
        this.pageData.problemList = [...this.pageData.problemList, ...(await this.$request('/AppWarning/AppWarning', 'GET', pageQuery)).data.data.rows.map(item => ({
