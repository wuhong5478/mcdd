      ...
      pageConfig: {
        handler: async function tmp () {
          ...
          this.pageData.problemList = (await this.$request('/AppFire/selectByCriteria', 'POST', {
            ...
            isTrue: (isTrueEnum.indexOf(this.pageConfig.navList[2]) === -1) ? undefined : isTrueEnum.indexOf(this.pageConfig.navList[2]),
            ways: (waysEnum.indexOf(this.pageConfig.navList[1]) === -1) ? undefined : waysEnum.indexOf(this.pageConfig.navList[1]) + 1
          })).data.data.map(item => ({
            device: item.devname,
            status: item.istrue,
